    ...
    <div className="mt-6 border bg-slate-100 rounded-md p-4">
        ...
        <>
          {initialData.attachments.length === 0 && (
            <p className="text-sm mt-2 text-slate-500 italic">
              {t("attachmentsForm.inputs-default.attachments")}
            </p>
          )}
          {initialData.attachments.length > 0 && (
            <div className="space-y-2">
              {initialData.attachments.map((attachment) => (
                ...
                >
                  <File className={cn("h-4 w-4 flex-shrink-0",dir(locale as Locale) === "rtl" ? " ml-2" : " mr-2")} />
                  ...
                  {deletingId !== attachment.id && (
                    <button
                      ...
                      className={cn("hover:opacity-75 transition", dir(locale as Locale) === "ltr" ?"ml-auto": "mr-auto")}
                    >
                      <X className="h-4 w-4" />
                    </button>
                  )}
      ...
      {isEditing && (
        <div>
          ...
          >
            {t("attachmentsForm.helper-texts.mainText")}
          </div>
        </div>
      )}
    </div>
